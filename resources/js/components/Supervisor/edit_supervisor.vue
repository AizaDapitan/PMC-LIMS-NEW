        ...
        <div>
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb breadcrumb-style1 mg-b-5">
              ...
              <li class="breadcrumb-item" aria-current="page">
                <a :href="dashboard">Supervisors</a>
              </li>
          ...
          <h4 class="mg-b-0 tx-spacing--1">Edit Supervisor</h4>
        </div>
      </div>
  ...
  export default {
    ...
    data() {
      return {
        errors_exist: false,
        errors: {},
        dashboard: this.$env_Url + "/supervisors/dashboard",
        form: {
          id: this.supervisor.id,
          name: this.supervisor.name,
          description: this.supervisor.description,
          active: true,
          checked: this.supervisor.active,
        },
      };
    },
    ...
    methods: {
      async update() {
        ...
        const res = await this.submit("post", "/supervisors/update", this.form, {
          headers: {
            "Content-Type":
        ...
        if (res.status === 200) {
          this.updmessage();
          window.location.href = this.$env_Url + "/supervisors/dashboard";
        } else {
