<template>
        ...
        <div>
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb breadcrumb-style1 mg-b-5">
              <li class="breadcrumb-item active" aria-current="page">LIMS</li>
              <li class="breadcrumb-item active" aria-current="page">Assistant QA Officers</li>
            </ol>
          </nav>
          <h4 class="mg-b-0 tx-spacing--1">Manage Assistant QA Officers</h4>
        </div>
      </div>
  ...
  export default {
    data() {
      return {
        filters: null,
        editMsg: "Edit Assistant QA Officer",
        assistants: [],
      };
    },
    created() {
    ...
    methods: {
      async fetchRecord() {
        const res = await this.getDataFromDB("get", "/assistantofficers/getAssistantOfficer");
      ...
      addNew() {
        window.location.href = this.$env_Url + "/assistantofficers/create";
      },
  
      edit(data) {
        let src = data.data.id,
          alt = data.data.id;
        window.location.href = this.$env_Url + "/assistantofficers/edit/" + alt;
      },
