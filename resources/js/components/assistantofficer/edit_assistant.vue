        ...
        <div>
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb breadcrumb-style1 mg-b-5">
              ...
              <li class="breadcrumb-item" aria-current="page">
                <a :href="dashboard">Assistant QA Officer</a>
              </li>
              <li class="breadcrumb-item active" aria-current="page">
                Edit Assistant QA Officer
              </li>
            </ol>
          </nav>
          <h4 class="mg-b-0 tx-spacing--1">Edit Assistant QA Officer</h4>
        </div>
      </div>
  ...
  export default {
    ...
    data() {
      return {
        errors_exist: false,
        errors: {},
        dashboard: this.$env_Url + "/assistantofficers/dashboard",
        form: {
          id: this.assistant.id,
          name: this.assistant.name,
          description: this.assistant.description,
          active: true,
          checked: this.assistant.active,
        },
      };
    },
    ...
    methods: {
      async update() {
        ...
        const res = await this.submit("post", "/assistantofficers/update", this.form, {
          headers: {
            "Content-Type":
        ...
        if (res.status === 200) {
          this.updmessage();
          window.location.href = this.$env_Url + "/assistantofficers/dashboard";
        } else {
