  ...
  <div class="container-fluid pd-x-0">
    ...
    >
      <div>
        ...
        <h4 class="mg-b-0 tx-spacing--1">Home/Worksheet - Assayer</h4>
      </div>
    </div>
              ...
              >
                <template #body="slotProps">
                  ...
                  <Button
                    ...
                    :disabled="slotProps.data.isApproved == 1 && !slotProps.data.is_reassay"
                  />
                </template>
              </Column>
            </DataTable>
  ...
  methods: {
    ...
    viewWorksheet(data) {
      let src = data.data.id,
        alt = data.data.id;
      window.location.href = this.$env_Url + "/assayer/view-worksheet/" + alt;
    },
    exportCSV() {
    ...
    editWorksheet(data) {
      let src = data.data.id,
        alt = data.data.id;
      window.location.href = this.$env_Url + "/assayer/edit-worksheet/" + alt;
    },
    deleteWorksheet(data) {
      this.$confirm.require({
        ...
        accept: async () => {
          const res = await this.deleteRecord("post", "/assayer/delete", {
            id: data.data.id,
          });
          if (res.status === 200) {
            this.rmessage();
