        ...
        <div>
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb breadcrumb-style1 mg-b-10">
              ...
              <li class="breadcrumb-item active" aria-current="page">Create Transmittal</li>
            </ol>
          </nav>
          <h4 class="mg-b-0 tx-spacing--1">Create Transmittal - Officer</h4>
        </div>
      </div>
  ...
  export default {
    ...
    methods: {
      ...
      async saveTransmittal() {
        ...
        const res = await this.submit("post", "/officer/store", form, {
          headers: {
            "Content-Type":
              "multipart/form-data; charset=utf-8; boundary=" +
              Math.random().toString().substr(2),
        ...
        if (res.status === 200) {
          window.location.href = this.$env_Url + "/officer/transmittal";
        } else {
          this.errors_exist = true;
          this.errors = res.data.errors;
          // this.ermessage(res.data.errors);
