<template>
        ...
        <div>
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb breadcrumb-style1 mg-b-5">
              ...
              <li class="breadcrumb-item active" aria-current="page">Fire Assayers</li>
            </ol>
          </nav>
          <h4 class="mg-b-0 tx-spacing--1">Manage Fire Assayers</h4>
        </div>
      </div>
  ...
  export default {
    ...
    methods: {
      async fetchRecord() {
        const res = await this.getDataFromDB("get", "/fireassayers/getFireAssayer");
      ...
      addNew() {
        window.location.href = this.$env_Url + "/fireassayers/create";
      },
  
      edit(data) {
        let src = data.data.id,
          alt = data.data.id;
        window.location.href = this.$env_Url + "/fireassayers/edit/" + alt;
      },
